...
### C. 구조 설계 (포맷: ${formatLabel}) — **1순위 세트 1개만 상세**, 나머지 4개는 "길이 + 훅 한 줄"
- 길이: 상위군 길이 분포에서 [행 n]
- 훅 3안 → 최적 1안 [원칙 ${hookPrinciple}]. 관찰이 있는 행은 [영상관찰 #n mm:ss]를 근거로, 없으면 첫 문장은 [가정]으로 표기
- (1순위 세트만) 타임라인 [원칙 ${structurePrinciple}] — 블록별 **목적과 장치**만. 대사 전문은 쓰지 않는다
- (1순위 세트만) 이탈 위험 구간 & 회수 장치 표 (구간 / 위험 신호 / 개입) — 시청지속률 데이터가 없으므로 전부 [가정] 또는 [원칙]
- 톤 & 보이스 3줄, 금지 리스트 3개 (세트 공통으로 한 번)
...
## 진행 방식 (답변 길이 때문에 세 번에 나눈다)
- **1단계**: 아래 "요청" 항목만 답하고 **멈춘다**. 시안(# 시안)은 아직 쓰지 않는다.
- **2단계**: 내가 "계속"이라고 하면 "# 시안"의 **A·B·F**만 쓰고 멈춘다. 1단계의 근거 태그를 그대로 인용한다.
- **3단계**: 내가 "이제 C, D, E"라고 하면 나머지를 쓴다.
- **웹 검색을 하지 않는다.** 이 문서 안의 표·관찰·원칙만으로 답한다. 없는 것은 "데이터 없음".
