...
1. **보관함 30일 만료** — 정책. 즉시. (history.ts, 1파일)
2. **0 unit 필드 3개 추가** — `paidProductPlacementDetails`, `topicCategories`, `defaultAudioLanguage`. 표에 PPL 열.
...
5. 프롬프트 "없는 것" 문구 정정 — 썸네일은 "없음"이 아니라 "첨부 방법", 자막은 "Gemini 경로 없으면"으로.
